...
function getQuadrant(position,middle){
  if(position[0]>middle[0]){ // low
    if(position[1]>middle[1]){ // right
      return 2;
    } else { //left
      return 3;
    }
...
function fillDeadEndsAndLogPossitions(mazeArray,keyPositions,doorPositions,quadrants,middle) {
    ...
    for (var i=0 ; i<mazeY ; i++ ) {
      for (var j=0 ; j<mazeX ; j++ ) {
          ...
          if(maze[i][j]==='.' || isDoor(maze[i][j])){
            if(findPossibleDirectionsToMove([i,j],maze).length<=1 && !(maze[i][j]==='@')){
              maze[i][j] = '#';
              removed++;
            } 
          }
          ...
          if (isKey(maze[i][j])){                
            ...
            quadrants[getCharIndex(maze[i][j])] = getQuadrant([i,j],middle);
          }
    }
  }
  return removed;
...
function doesLeftKeyContain(leftKeys,keyIndex){
  ...
  function getPossibilitiesFromMatching(leftKeys,blockings,currentKeyIndex = 27,distances,myLastMoveInEachQuadrant,quadrants){
    let posArray = []
    ...
    leftKeys.forEach((element,index)=>{
      ...
      if (element && blockings[index].length<=1){
        console.log('i want to move to:', String.fromCharCode(97+index), 'and im standing on:', String.fromCharCode(97+currentKeyIndex) )
        console.log(quadrants[index]);
        console.log(myLastMoveInEachQuadrant[quadrants[index]])
        console.log(distances[myLastMoveInEachQuadrant[quadrants[index]]][index]);
        if(distances[myLastMoveInEachQuadrant[quadrants[index]]][index]=== 0) debugger;
        posArray.push([index,distances[myLastMoveInEachQuadrant[quadrants[index]]][index]]);
  ...
  function getDistanceToCollectKeys(keyPosition,keyIndex,blockings,leftKeys,distances,cache,keyPath,distanceToHere,lastMoveInEachQuadrant,quadrants){
    let myLeftKeys = leftKeys.slice(0);
    let myKeyPath = keyPath.slice(0);
    let myLastMoveInEachQuadrant = lastMoveInEachQuadrant.slice(0);
    ...
    let posibilities = getPossibilitiesFromMatching(myLeftKeys,myBlockings,keyIndex,distances,myLastMoveInEachQuadrant,quadrants);
    ...
    posArray.forEach(element=>{
      let nextlastMoveInEaxhQuadrant = myLastMoveInEachQuadrant.slice(0);
      nextlastMoveInEaxhQuadrant[quadrants[element[0]]] = element[0];
      let getDistance = getDistanceToCollectKeys(myLeftKeys[element[0]], element[0],myBlockings,myLeftKeys,distances,cache,myKeyPath,element[1],nextlastMoveInEaxhQuadrant,quadrants);
    ...
    export function solveMaze(mazeInput,startingPosition = [40,40],numberOfTotalKeys = 26) {
      ...
      let keyPath = [0];
      let quadrants = new Array(26)
      let possiblePaths = []
      fillDeadEndsAndLogPossitions(mazeArray,keyPositions,doorPositions,quadrants,startingPosition) // mutating the array.
      ...
      positions.forEach(element=>{
        mapDistances(mazeArray,distances,keyPositions,element);
      });
        //create a map for path to each key and retreive blockers.
        positions.forEach(element=>{

      possiblePaths = possiblePaths.concat(useEcho(mazeArray,element,0).possiblePaths);
    });
      ...
      // (keyPosition,keyIndex,blockings,leftKeys,distances,cache,keyPath,distanceToHere,lastMoveInEachQuadrant,quadrants)      //(keyPosition,keyIndex,blockings,leftKeys,distances,cache)
      console.log(quadrants);
      console.log(blockings);
      console.log(distances)
      debugger;
      let lastMoveInEachQuadrant = [27,27,27,27];
      let shortestDistance = getDistanceToCollectKeys(startingPosition,undefined,blockings,keyPositions,distances,cache,keyPath,0,lastMoveInEachQuadrant,quadrants)
