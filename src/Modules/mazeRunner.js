    ...
    function findPossibleDirectionsToMove(position){
      ...
      function isDoor(char){
        ...
        // }
          ...
          if(nextKeyPossibilities.length<=0){
            console.log(stepsUntillNow);
            treeLengthLog.unshift(stepsUntillNow);
            return;
          } else {
            nextKeyPossibilities.sort((a,b)=> a[1]-b[1])
            nextKeyPossibilities.forEach(element=>{
...
function findNextKeyPossibilities(nextKeyPossibilities,mazeArray,position,openDoorsArray,foundKeysArray,steps){
  ...
  function isKey(char){
    ...
    export function fillDeadEnds(mazeArray,startingPosition) {
      let maze = mazeArray.map(function(arr) {
        return arr.slice();
      });
      let existingDoors = []
      let existingKeys = []
        ...
        for (var i=0; i<mazeY;i++){
          for(var j=0;j<mazeX;j++){
            if(maze[i][j]==='.' || isDoor(maze[i][j])){
              if(findPossibleDirectionsToMove([i,j]).length>=3){
                maze[i][j] = '#';
                removed++;
      ...
      echo(possiblePaths,mazeForEcho,startingPosition,0,[]);
      ...
      possiblePathArrays.map(element =>{
        removeOnlyKeys(element);
      });
