...
function parseWire(element) {
  ...
  function extreameParseWire(array) {
    ...
    array.forEach(element =>{
      ...
      let segmentLength = Number(element.slice(1));
      let newSegment = new Array(segmentLength);
      for (var i = 0; i < segmentLength; i++){
        newSegment[i] = movmementType;
      }
      newArray = newArray.concat(newSegment);
    })
    return newArray
  }

export const wireArrayA = wireA.split(',').map(parseWire);
export const wireArrayB = wireB.split(',').map(parseWire);


// export const wireArrayA = extreameParseWire(wireA.split(','));
// export const wireArrayB = extreameParseWire(wireB.split(','));
